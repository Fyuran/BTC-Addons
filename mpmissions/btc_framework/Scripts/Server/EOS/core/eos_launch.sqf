if (isServer) then {
	...
	private _errorFlag = _JIPmkr findIf {markerPos _x isEqualTo [0,0,0]};
	if(_errorFlag != -1) exitWith {["EOS",format["EOS_Spawn: bad or nonexistant markers found %1", _JIPmkr]] call btc_fnc_logDebug};
...

	{
		...
		null = [_x,[_HPpatrols,_HPgroupArray],[_PApatrols,_PAgroupArray],[_LVehGroups,_LVgroupArray],[_AVehGroups,_SVehGroups,_CHGroups,_CHgroupArray],_settings] execVM "Scripts\Server\eos\core\eos_core.sqf";
	}foreach _JIPmkr;
};