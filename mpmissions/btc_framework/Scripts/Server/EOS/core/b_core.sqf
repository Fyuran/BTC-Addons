...
private ["_fGroup","_cargoType","_vehType","_CHside","_mkrAgl","_initialLaunch","_pause","_eosZone","_hints","_waves","_aGroup","_side","_enemyFaction","_mAH","_mAN","_distance","_grp","_cGroup","_bGroup","_CHType","_time","_timeout","_faction"];
...
_basActivated setTriggerArea [_mkrX,_mkrY,_mkrAgl,FALSE, [-1,20] select _heightLimit];
_basActivated setTriggerActivation ["ANY","PRESENT",true];
_basActivated setTriggerStatements ["this","",""];
...
_bastActive setTriggerArea [_mkrX,_mkrY,_mkrAgl,FALSE, [-1,20] select _heightLimit];
_bastActive setTriggerActivation ["any","PRESENT",true];
_bastActive setTriggerTimeout [1, 1, 1, true];
_bastActive setTriggerStatements ["this","",""];
...
//SPAWN HELICOPTERS
	...
	if ((_fSize select 0) > 0) then {
		_cargoGrp = createGroup [_side,true];
			...
			0=[_cargoGrp,"INFskill"] call eos_fnc_grouphandlers;
				...
				null = [_mkr,_fGroup,_counter] execvm "Scripts\Server\eos\functions\TransportUnload_fnc.sqf";
	...
	sleep 1;
			if (!triggeractivated _bastActive || getmarkercolor _mkr == "colorblack") exitwith
			{
					...
					_mkr setmarkeralpha _mAN;
						...
						if (_eosZone) then {
null = [_mkr,[_PApatrols,_PAgroupSize],[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,0,0,0],[_faction,_mA,350,_CHside]] execVM "Scripts\Server\eos\core\eos_core.sqf";
...
null = [_mkr,[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups],[_CHGroups,_fSize],_settings,[_pause,_waves,_timeout,_eosZone,_hints],true] execVM "Scripts\Server\eos\core\b_core.sqf";
						};};
