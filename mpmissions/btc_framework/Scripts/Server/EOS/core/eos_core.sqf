...
if (!_cache) then {
		...
		_eosActivated setTriggerArea [(_distance+_mkrX),(_distance+_mkrY),_mkrAgl,FALSE, [-1,500] select _heightLimit];
...
//SPAWN ALT TRIGGERS
			...
			_clear setTriggerStatements ["this","",""];
				...
				_taken setTriggerActivation ["ANYPLAYER","PRESENT",true];
				_taken setTriggerStatements ["this","",""];
_eosAct=true;
while {_eosAct} do
	{
...
if (!(getmarkercolor _mkr == "colorblack")) then {
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "Scripts\Server\eos\core\eos_core.sqf";
