...
ENGIMA_CIVILIANS_StartCivilians = {
	...
	_enemyUnitData= [_this, "ENEMY_UNIT_CLASSES", [["C_man_hunter_1_F"],["arifle_Mk20_plain_F"]]] call ENGIMA_CIVILIANS_GetParamValue;
	...
	_spawnUnit = {
		...
		if (count _pos > 0) then {
			...
			[_unit,_enemyUnitData] call _fnc_OnSpawnCallback;
		};
		_unit
	};
...
BTC_ENGIMA_Enemy_CQB_Spawn = {
    params[
    	["_unit", objNull,[objNull]],
		["_enemyUnitData",[],[[]]]
    ];
	if(_enemyUnitData isEqualTo []) exitWith {["BTC_ENGIMA_Enemy_CQB_Spawn: _enemyUnitData is empty"] call BIS_fnc_error};
	btc_engima_enemy_cqb_group = createGroup [east,true];
	_delay = if(BTC_DEBUG) then {10} else {240}; //for testing purposes
	[{

		params["_unit", "_enemyUnitData"];
		_enemyUnitData params ["_enemyUnitClasses", "_enemyUnitWeaponClasses"];
		_cfg = configfile;

		//join group
	    [_unit] join btc_engima_enemy_cqb_group;
	    btc_engima_enemy_cqb_group setBehaviour "STEALTH";
	    _unit setVariable ["isBTC_ENGIMA_Enemy_CQB",true];

	    //Weapon & Magazine
	    _weapon = selectRandom _enemyUnitWeaponClasses;
	    _magazine = selectRandom(getArray (_cfg >> "CfgWeapons" >> _weapon >> "magazines"));
	    _unit addWeapon _weapon;
	    _unit addMagazine _magazine;
	    _unit addMagazine _magazine;

	    //Sound
	    _sound = getArray (_cfg >> "CfgWeapons" >> _weapon >> "reloadMagazineSound") select 0;
	    playSound3D [_sound + ".wss", objNull, false, getPosASL _unit, 1, 1, 50];

	    if(BTC_DEBUG) then {
			["BTC_ENGIMA_Enemy_CQB_Spawn", format["enemy civilian unit: %1, at %2, with weapon %3", _unit, getPos _unit, _weapon]] call btc_fnc_logDebug;
		};
	}, _this, _delay] call CBA_fnc_waitAndExecute;
};

BTC_ENGIMA_Enemy_CQB_deSpawn = {
    params[["_unit", objNull,[objNull]]];
	btc_engima_enemy_cqb_group = grpNull;
    if(BTC_DEBUG) then {
		["BTC_ENGIMA_Enemy_CQB_deSpawn", format["enemy civilian unit removed: %1", _unit]] call btc_fnc_logDebug;
	};
};