/* ----------------------------------------------------------------------------
Function: btc_fnc_side_bombdefusal_nuke_fail
...
Examples:
    (begin example)
        _result = [] call btc_fnc_side_bombdefusal_nuke_fail;
    (end)
...
params[
	...
	["_destructionRadius",0,[0]],
	["_destructionRate",0,[0]],
	["_doColorCorrection",false,[false]]
];
if(_center isEqualType objNull) then {_center = getPosATL _center};

if (BTC_DEBUG) then {
    ["btc_fnc_side_bombdefusal_nuke_fail",format["nuke at %1", _center]] call btc_fnc_logDebug;
};
...
[[_center, _destructionRadius, _destructionRate, _doColorCorrection],
{
	...
	playSound "btc_sound_distant_nuke";
	// Mushroom cloud
